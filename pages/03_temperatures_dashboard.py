...
def load_data():
    ...
    temps_df = pd.read_csv(data_path)  # TODO: Ex 3.1: Load the dataset using Pandas, use the data_path variable.
...
# Find the row index with the minimum temperature
...
# Find the row index with the maximum temperature
...
# Calculate the list of unique cities to use as options
...
if unique_cities_list is not None:
    ...
    # Fixed: Used 'unique_cities_list' instead of 'unique_countries_list' to match the default cities
...
if unique_cities_list is not None and len(selected_cities) > 0:
    ...
    # Fixed: These lines are now outside the for loop
